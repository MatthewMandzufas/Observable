...
describe('take', () => {
  ...
  it.skip('should take one value of an observable with one value', () => {
    testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      const e1 = hot('  ---(a|)');
      const e1subs = '  ^--!---';
      const expected = '---(a|)';
  ...
  it.skip('should work with throw', () => {
    testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
      const e1 = cold(' #');
      const e1subs = '  (^!)';
      const expected = '#';
  ...
  it.skip('should not break unsubscription chain when unsubscribed explicitly', () => {
    testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      const e1 = hot('---^--a--b-----c--d--e--|');
      const unsub = '    ---------!            ';
      const e1subs = '   ^--------!            ';
  ...
  it.skip('should unsubscribe from the source when it reaches the limit', () => {
  ...
  it.skip('should complete when the source is reentrant', () => {
    let completed = false;
  ...
  it.skip('should complete even if the parameter is a string', () => {
    testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
      const e1 = cold(' --a-----b----c---d--|');
      const e1subs = '  ^-------!------------';
      const expected = '--a-----(b|)         ';
  ...
  it.skip('should unsubscribe from the source when it reaches the limit before a recursive synchronous upstream completion is notified', () => {
    testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
      const subject = new Subject();
      const e1 = cold(' (a|)');
      const e1subs = '  (^!)';
  ...
  it.skip('should properly handle reentrancy when taking many values', () => {
    const results: any[] = [];
    ...
    expect(results).toEqual([5, 'done', 'finalize source', 'finalize result']);
  });
